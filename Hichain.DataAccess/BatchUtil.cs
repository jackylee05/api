...
namespace Hichain.DataAccess.Data
{
    public static class BatchUtil
    {
        ...
        public static (string, List<object>) GetSqlDelete(IQueryable query, DbContext context)
        {
            ...
            {
                resultQuery = $"{leadingComments}DELETE {topStatement}{tableAlias}{sql}";
                resultQuery = resultQuery.ToLower();
                resultQuery = resultQuery.Replace(string.Format(" as {0}", tableAlias.ToString().ToLower()), " ");
                resultQuery = resultQuery.Replace(string.Format("{0}.", tableAlias.ToString().ToLower()), "");
                resultQuery = resultQuery.Replace(string.Format("delete {0}\r\n", tableAlias.ToString().ToLower()), "delete \r\n");
            }
            else
            {
                resultQuery = $"{leadingComments}DELETE {topStatement}{tableAlias}{sql}";
        ...
        {
            var tableInfo = TableInfoEx.CreateInstance(context, updateValuesType, new List<object>(), OperationType.Read, new BulkConfig());
        ...
        private static string GetSqlSetSegment(DbContext context, TableInfoEx tableInfo, Type updateValuesType, object updateValues, object defaultValues, List<string> updateColumns, List<object> parameters)
        {
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        private static void AddSqlParameter(StringBuilder sqlColumns, List<object> sqlParameters, TableInfoEx tableInfo, string columnName, object value)
        {
        ...
        public static DbParameter TryCreateRelationalMappingParameter(string columnName, string parameterName, object value, TableInfoEx tableInfo)
        {
            if (columnName == null)
                return null;
        ...
        public static bool TryCreateUpdateBodyNestedQuery(BatchUpdateCreateBodyData createBodyData, Expression expression, MemberAssignment memberAssignment)
        {
            ...
            var fk = firstNavigation.ForeignKey;
            if (fk.DependentToPrincipal != null)
            {
                for (int keyIndex = 0; keyIndex < dependencyKeyProperties.Count; ++keyIndex)
                {
                    ...
                    var dependencyColumnName =
                        navigationColumnFastLookup[dependencyKeyProperties[keyIndex].Name];

                    var principalColumnName =
                        columnNameValueDict[principalKeyProperties[keyIndex].Name];

                    whereClauseCondition
                        .Append(firstNavigationAlias)
                        .Append(".[")
                        .Append(principalColumnName)
                        .Append("] = [")
                        .Append(rootTableAlias)
                        .Append("].[")
                        .Append(dependencyColumnName)
                        .Append(']');
                }
            }
            else
