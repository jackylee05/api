...
namespace Hichain.DataAccess.Data
{
    public class SqlServerAdapter : ISqlOperationsAdapter
    {
        #region Methods
        // Insert
        public void Insert<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress)
        {
            InsertAsync(context, type, entities, tableInfo, progress, CancellationToken.None, isAsync: false).GetAwaiter().GetResult();
        }

        public async Task InsertAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress, CancellationToken cancellationToken)
        {
        ...
        protected async Task InsertAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress, CancellationToken cancellationToken, bool isAsync)
        {
        ...
        // Merge
        public void Merge<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, OperationType operationType, Action<decimal> progress) where T : class
        {
        ...
        public async Task MergeAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, OperationType operationType, Action<decimal> progress, CancellationToken cancellationToken) where T : class
        {
        ...
        protected async Task MergeAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, OperationType operationType, Action<decimal> progress, CancellationToken cancellationToken, bool isAsync) where T : class
        {
        ...
        // Read
        public void Read<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress) where T : class
        {
            ReadAsync(context, type, entities, tableInfo, progress, CancellationToken.None, isAsync: false).GetAwaiter().GetResult();
        }

        public async Task ReadAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress, CancellationToken cancellationToken) where T : class
        {
            await ReadAsync(context, type, entities, tableInfo, progress, cancellationToken, isAsync: true).ConfigureAwait(false);
        }

        protected async Task ReadAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress, CancellationToken cancellationToken, bool isAsync) where T : class
        {
            ...
            try
            {
                ...
                if (typeof(T) == type)
                {
                    ...
                    var compiled = Microsoft.EntityFrameworkCore.EF.CompileQuery(expression); // instead using Compiled queries
        ...
        // Truncate
        public void Truncate(DbContext context, TableInfoEx tableInfo)
        {
        ...
        public async Task TruncateAsync(DbContext context, TableInfoEx tableInfo, CancellationToken cancellationToken)
        {
            var sqlTruncateTable = SqlQueryBuilder.TruncateTable(tableInfo.FullTableName);
        ...
        //    return context.GetUnderlyingConnection(config);
        //}
        internal static async Task<DbConnection> OpenAndGetSqlConnectionAsync(
    DbContext context,
    BulkConfig config,
    CancellationToken cancellationToken)
        ...
        //    }
        //}
        private static Microsoft.Data.SqlClient.SqlBulkCopy GetSqlBulkCopy(
    ...
    BulkConfig config)
        {
            ...
            if (transaction == null)
            {
                return new Microsoft.Data.SqlClient.SqlBulkCopy(
                    sqlConnection,
                    sqlBulkCopyOptions,
                    null);
            }
            ...
            {
                var sqlTransaction =
                    (Microsoft.Data.SqlClient.SqlTransaction)transaction.GetDbTransaction();
        ...
        /// <param name="tableInfo"></param>
        /// <returns></returns>
        private static DataTable InnerGetDataTable<T>(DbContext context, ref Type type, IList<T> entities, TableInfoEx tableInfo)
        {
            var dataTable = new DataTable();
            var columnsDict = new Dictionary<string, object>();
            var ownedEntitiesMappedProperties = new HashSet<string>();

            var isSqlServer = context.Database.ProviderName.EndsWith(DatabaseType.SqlServer.ToString());
            //var sqlServerBytesWriter = new SqlServerBytesWriter();
            SqlServerBytesWriter sqlServerBytesWriter = null; // 删除所有引用
            ...
            foreach (var property in properties)
            {
                if (entityPropertiesDict.ContainsKey(property.Name))
                {
                    ...
                    if (isSqlServer && propertyType.FullName?.Contains("HierarchyId") == true)
                    {
                        propertyType = typeof(byte[]);
                    }
                ...
                else if (entityNavigationOwnedDict.ContainsKey(property.Name)) // isOWned
                {
                    ...
                    if (ownedEntityType == null)
                    {
                        ...
                        ownedEntityType = entityNavigationOwnedDict[property.Name].TargetEntityType;
                    }
            ...
            foreach (var entity in entities)
            {
                foreach (var property in properties)
                {
                    ...
                    //}
                    if (isSqlServer && propertyValue != null && propertyValue.GetType().FullName?.Contains("HierarchyId") == true)
                    {
                        using var memStream = new MemoryStream();
                        using var binWriter = new BinaryWriter(memStream);
                        dynamic hierarchyValue = propertyValue;
                        hierarchyValue.Write(binWriter);
                        propertyValue = memStream.ToArray();
                    }
        ...
        private static string GetDiscriminatorColumn(TableInfoEx tableInfo)
        {
