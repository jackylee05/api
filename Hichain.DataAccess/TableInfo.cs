...
namespace Hichain.DataAccess.Data
{
    public class TableInfoEx
    {
        ...
        public static TableInfoEx CreateInstance<T>(DbContext context, Type type, IList<T> entities, OperationType operationType, BulkConfig bulkConfig)
        {
            var tableInfo = new TableInfoEx
            {
        ...
        public void LoadData<T>(DbContext context, Type type, IList<T> entities, bool loadOnlyPKColumn)
        {
            ...
            bool isSqlServer = context.Database.ProviderName.EndsWith(DatabaseType.SqlServer.ToString());
                    ...
                    foreach (var navgationProperty in ownedTypes)
                    {
                        ...
                        if (ownedEntityType == null) // when entity has more then one ownedType (e.g. Address HomeAddress, Address WorkAddress) or one ownedType is in multiple Entities like Audit is usually.
                        {
                            ...
                            var navigation = entityType
                             ...
                             ownedEntityType = navigation?.TargetEntityType;
                        }
        ...
        #region SqlCommands
        public async Task<bool> CheckTableExistAsync(DbContext context, TableInfoEx tableInfo, CancellationToken cancellationToken, bool isAsync)
        {
            if (isAsync)
            {
        ...
        protected void UpdateEntitiesIdentity(DbContext context, TableInfoEx tableInfo, IList<object> entities, IList<object> entitiesWithOutputIdentity)
        {
        ...
        public async Task LoadOutputDataAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, CancellationToken cancellationToken, bool isAsync) where T : class
        {
        ...
        {
            var compiled = Microsoft.EntityFrameworkCore.EF.CompileQuery(GetQueryExpression(type, sqlQuery));
