...
namespace Hichain.DataAccess.Data
{
    internal static class DbContextBulkTransactionGraphUtil
    {
        ...
        private static async Task ExecuteWithGraphAsync(DbContext context, IEnumerable<object> entities, OperationType operationType, BulkConfig bulkConfig, Action<decimal> progress, CancellationToken cancellationToken, bool isAsync)
        {
            ...
            if (SqlAdaptersMapping.GetDatabaseType(context) == DatabaseType.MySql)
            ...
            try
            {
                foreach (var actionGraphItem in rootGraphItems)
                {
                    ...
                    var tableInfo = TableInfoEx.CreateInstance(context, actionGraphItem.EntityClrType, entitiesToAction, operationType, bulkConfig);
                    ...
                    foreach (var graphEntity in actionGraphItem.Entities)
                    {
                        ...
                        if (parentEntity is null)
                        {
                            foreach (var navigation in actionGraphItem.Relationships)
                            {
                                ...
                                if (navigation.ParentNavigation.ForeignKey.DependentToPrincipal == null)
                                {
                        ...
                        else
                        {
                            ...
                            if (navigation.ForeignKey.DependentToPrincipal == null)
                            {
