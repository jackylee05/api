...
namespace GP.Framework.DataAccess.Data
{
    ...
    internal static class SqlBulkOperation
    {
        ...
        public static void Insert<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress)
        {
        ...
        public static async Task InsertAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress, CancellationToken cancellationToken)
        {
            var adapter = SqlAdaptersMapping.CreateBulkOperationsAdapter(context);
            await adapter.InsertAsync(context, type, entities, tableInfo, progress, cancellationToken);
        }

        public static void Merge<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, OperationType operationType, Action<decimal> progress) where T : class
        {
            var adapter = SqlAdaptersMapping.CreateBulkOperationsAdapter(context);
            adapter.Merge(context, type, entities, tableInfo, operationType, progress);
        }

        public static async Task MergeAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, OperationType operationType, Action<decimal> progress, CancellationToken cancellationToken) where T : class
        {
            var adapter = SqlAdaptersMapping.CreateBulkOperationsAdapter(context);
            await adapter.MergeAsync(context, type, entities, tableInfo, operationType, progress, cancellationToken);
        }

        public static void Read<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress) where T : class
        {
        ...
        public static async Task ReadAsync<T>(DbContext context, Type type, IList<T> entities, TableInfoEx tableInfo, Action<decimal> progress, CancellationToken cancellationToken) where T : class
        {
            if (tableInfo.BulkConfig.UseTempDB) // dropTempTableIfExists
            {
        ...
        public static void Truncate(DbContext context, TableInfoEx tableInfo)
        {
            var adapter = SqlAdaptersMapping.CreateBulkOperationsAdapter(context);
            adapter.Truncate(context, tableInfo);
        }

        public static async Task TruncateAsync(DbContext context, TableInfoEx tableInfo, CancellationToken cancellationToken)
        {
            var adapter = SqlAdaptersMapping.CreateBulkOperationsAdapter(context);
