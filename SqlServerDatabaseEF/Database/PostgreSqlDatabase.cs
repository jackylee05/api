...
{
    /// <summary>
    /// Defines the <see cref="PostgreSqlDatabase" />.
    /// </summary>
    public class PostgreSqlDatabase : IDatabase
    {
        ...
        {
            dbContext = new PostgreDbContext(connString);
        }
        ...
        public async Task<(int total, IEnumerable<T>)> FindList<T>(string strSql, DbParameter[] dbParameter, string sort, bool isAsc, int pageSize, int pageIndex) where T : class
        {
            using (var dbConnection = dbContext.Database.GetDbConnection())
            {
                ...
                if (total > 0)
                {
                    var reader = await dbHelper.ExecuteReadeAsync(CommandType.Text, sb.ToString().ToLower(), dbParameter);
