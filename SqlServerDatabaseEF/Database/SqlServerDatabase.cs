...
namespace Hichain.DataAccess.Data.EF
{
    public class SqlServerDatabase : IDatabase
    {
        ...
        public async Task<IEnumerable<T>> FindList<T>(string strSql, DbParameter[] dbParameter) where T : class
        {
            ...
            {
                var reader = await new DbHelper(dbContext, dbContext.Database.GetDbConnection(), dbContextTransaction).ExecuteReadeAsync(CommandType.Text, strSql, dbParameter);
                return DatabasesExtension.IDataReaderToList<T>(reader);
            }
        }
        ...
        {
            string strsql = string.Format("SELECT MAX({0}) FROM {1} WHERE {2}", seqcol, tablename, strwhere);
